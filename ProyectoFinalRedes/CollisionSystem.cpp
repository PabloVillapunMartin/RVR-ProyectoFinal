...
void CollisionSystem::fixCollision(Transform* &player, Transform* &wall){
	...
	if(wall->position_.getY() <= player->position_.getY() && (wall->position_.getY() + wall->height_) >= (player->position_.getY() + player->height_)){
		//Derecha
		if((player->position_.getX() + player->width_) >= wall->position_.getX()) colisionDerecha(player, wall);
		//Izquierda
	...
	//colision vertical
		...
		//Arriba
		if((player->position_.getY() + player->height_) >= wall->position_.getY()) colisionArriba(player, wall);
		//Abajo
